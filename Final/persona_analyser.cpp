...
void printBarChart(ofstream& out, const map<string, int>& traitTotals) {
    const vector<string> order = {
        "Neuroticism", "Openness", "Extraversion", "Agreeableness", "Conscientiousness"
    };
 
    const int BAR_WIDTH = 40;
    const int LOW_DIV   = 16;
    const int MOD_DIV   = 27;
 
    out << "\n";
    out << "  +======================================================+\n";
    out << "  ||        PERSONALITY TRAIT PROFILE - BAR CHART       ||\n";
    out << "  ||             Score Range: 5 (Low) to 20 (High)      ||\n";
    out << "  +======================================================+\n";
    out << "\n";
    out << "                       5    7    9   11   13   15   17   19  20\n";
    out << "                       |    |    |    |    |    |    |    |   |\n";
    out << "                       [====LOW=====][===MODERATE===][===HIGH===]\n";
    out << "\n";
 
    for (const string& trait : order) {
        auto it = traitTotals.find(trait);
        int score = (it != traitTotals.end()) ? it->second : 5;
        string band = getBand(score);
 
        int filled = ((score - 5) * BAR_WIDTH) / 15;
        ...
        for (int i = 0; i < filled; i++) {
            if (i == LOW_DIV || i == MOD_DIV) continue;
            if      (i < LOW_DIV) bar[i] = '#';
            else if (i < MOD_DIV) bar[i] = '=';
            else                  bar[i] = '*';
        }
 
        string scoreStr  = to_string(score) + "/20";
        string pct       = "(" + to_string((score * 100) / 20) + "%)";
        string bandLabel;
        if      (band == "LOW")      bandLabel = "[ LOW      ]";
        else if (band == "MODERATE") bandLabel = "[ MODERATE ]";
        else                         bandLabel = "[ HIGH     ]";
 
        out << "  " << padRight(trait, 17)
            << " |" << bar << "|"
            << " " << padLeft(scoreStr, 5)
            << " " << padLeft(pct, 5)
            << "  " << bandLabel << "\n";
    ...
    out << "                       5          9  10          15  16         20\n";
    out << "\n";
    out << "  LEGEND:  ### LOW (5-9)   === MODERATE (10-15)   *** HIGH (16-20)\n";
    out << "  +======================================================+\n\n";
}
 
void generateReport(ofstream& out,
...
public:
    ...
    void deleteQuestion() {
        ...
        // Show all questions with their IDs
        cout << "\n===== Available Questions =====\n";
        for (const Question& q : questions)
            cout << "  ID: " << q.questionID
                 << " | [" << q.trait << "] "
                 << q.questionText << "\n";
        ...
        // ── Renumber IDs per trait to close the gap ──────────────────
    ...
    // ── Calculate scores by reading each candidate's individual response file ──
    void calculateScores() {
        // Read the registry to find all candidate files
        ...
        // Each line in registry: candidateID|candidateName|fileName
        while (getline(reg, registryLine)) {
            registryLine = trim(registryLine);
            if (registryLine.empty()) continue;
 
            // Parse registry line
            size_t p1 = registryLine.find('|');
            size_t p2 = registryLine.find('|', p1 + 1);
            if (p1 == string::npos || p2 == string::npos) continue;
 
            string regID   = trim(registryLine.substr(0, p1));
            string regName = trim(registryLine.substr(p1 + 1, p2 - p1 - 1));
            string regFile = trim(registryLine.substr(p2 + 1));
 
            // Open the individual candidate response file
            ifstream candFile(regFile);
            if (!candFile) {
                cout << "Warning: Could not open file for candidate " << regID << " (" << regFile << ")\n";
                continue;
            }
 
            // Parse the candidate response file
            ...
            while (getline(candFile, line)) {
                line = trim(line);
                if (line.substr(0, 12) == "CandidateID:")
                    candidateID   = trim(line.substr(12));
                else if (line.substr(0, 5) == "Name:")
                    candidateName = trim(line.substr(5));
                else if (line.substr(0, 4) == "Age:")
                    candidateAge  = trim(line.substr(4));
                else if (line.substr(0, 6) == "Phone:")
                    candidatePhone= trim(line.substr(6));
                else if (line.substr(0, 7) == "Trait: ")
                    currentTrait  = trim(line.substr(7));
                else if (line.substr(0, 6) == "Marks:") {
                    try {
                        int m = stoi(trim(line.substr(6)));
                        if (!currentTrait.empty())
                            traitTotals[currentTrait] += m;
                    } catch (...) {}
                }
            }
            candFile.close();
            ...
            // Write score summary + full report to candidate_scores.txt
            generateReport(scoreOut, candidateID, candidateName,
        ...
        if (candidatesProcessed == 0)
            ...
            cout << "\nScores + reports for " << candidatesProcessed
                 << " candidate(s) written to " << scoreFile << ".\n";
    }
};
...
private:
    ...
    // Register candidate in the registry file for later score calculation
    void registerCandidate(const string& fileName) {
        ofstream reg("candidate_registry.txt", ios::app);
        if (reg)
            reg << candidateID << "|" << candidateName << "|" << fileName << "\n";
        reg.close();
    }
...
public:
    void startAssessment() {
        ...
        cout << "  ---------------------------------------------------------------\n";
        cout << "  This is a personality assessment based on the internationally\n";
        cout << "  recognized Big Five (OCEAN) psychological framework, adapted\n";
        cout << "  for Nepal Army candidate evaluation.\n";
        cout << "\n";
        cout << "  PURPOSE\n";
        cout << "  ---------------------------------------------------------------\n";
        cout << "  The test is designed to understand your character, behavioral\n";
        cout << "  tendencies, and personality strengths. Results are reviewed\n";
        cout << "  by administrators only and used to better understand each\n";
        cout << "  candidate's profile during the selection process.\n";
        cout << "\n";
        cout << "  WHAT IS BEING MEASURED\n";
        cout << "  ---------------------------------------------------------------\n";
        cout << "  Your responses will be assessed across 5 personality attributes:\n";
        cout << "\n";
        cout << "    1. Neuroticism       - Emotional stability under pressure\n";
        cout << "    2. Openness          - Adaptability and openness to change\n";
        cout << "    3. Extraversion      - Social confidence and communication\n";
        cout << "    4. Agreeableness     - Cooperation and teamwork ability\n";
        cout << "    5. Conscientiousness - Discipline, duty and reliability\n";
        cout << "\n";
        cout << "  TEST STRUCTURE\n";
        cout << "  ---------------------------------------------------------------\n";
        cout << "    - Total Attributes   : 5\n";
        cout << "    - Questions per Attr : 5\n";
        ...
        // Register this candidate for later score calculation
        ...
        // ── Sort responses by QuestionID before saving ────────────────────
        ...
        // ── Write sorted responses to file ────────────────────────────────
        ...
        switch (mainChoice) {
            ...
            case 1: {
                ...
                do {
                    ...
                    cout << "4. Calculate Candidate Scores\n";
                    cout << "5. Logout\n";
                    cout << "Enter choice: ";
                    ...
                    switch (adminChoice) {
                        ...
                        case 3: admin.deleteQuestion();  break;  // NEW
                        case 4: admin.calculateScores(); break;
                        case 5: cout << "\nLogout Successful.\n"; break;
                        default: cout << "\nInvalid Choice!\n";
                    }
                } while (adminChoice != 5);  // updated from 4 to 5
                break;
            }
